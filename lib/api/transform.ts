...
// Open-Meteo repeats a name across fields for some places (e.g. Tokyo, whose
// admin1 is also "Tokyo"), so drop duplicate parts when composing the label.
function composeLabel(parts: Array<string | undefined>): string {
...
export function toSearchResults(upstream: GeocodingResponse): SearchResult[] {
...
export function nominatimToSearchResults(places: NominatimSearch): SearchResult[] {
  return places.map(p => {
    const a = p.address;
    const name =
      a?.city ??
...
// Reduce a Nominatim reverse result to a single settlement name, preferring the
// most specific locality that's present. Returns "" when nothing usable is found,
// letting the caller fall back to the timezone-derived city.
export function reverseToLabel(upstream: NominatimReverse): string {
